...
app.get("/movies/:id",async (request,response) => {
  ...
  movie ? response.send(movie) : response.status(404).send("Movie not available");
})
...
app.delete("/movies/:id",async(request,response) => {
  ...
  result.deletedCount >= 1 ? response.send({message : "Movie deleted successfully"}) : response.status(404).send({message : "Movie already deleted"});
